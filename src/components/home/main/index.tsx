...
export default function Main() {
  ...
  const Link = (data: any) => {
    return (
      <button
        className="flex flex-col db-border items-center py-[12px]  w-full"
        onClick={async () => {
          const url = data?.isVerified ? false : await GetVerificationLink("google-login");
...
<div className="flex flex-col w-full md:w-3/4 mx-auto min-h-screen gap-4">
      ...
      providers={
     [
      spotifyProofCheck + instagramProofCheck
     ]
      ...
      links={ 
        <>
          { 
            !spotifyProofCheck && 
            <LinkBtn platform={"spotify"} link={async () => {
            ...
            /> 
              ...
              !instagramProofCheck && 
              <LinkBtn platform={"instagram"} link={async () => {
  ...
  return (
    <span className="flex w-full md:p-14 p-2 md:-mt-12">
      <span className="flex justify-between w-full items-center">
        ...
        <button className="flex items-center justify-center bg-black text-white rounded-full px-4 py-1 mt-4 "
                onClick={() => {
