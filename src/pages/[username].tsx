...
export default function Home() {
  ...
  const fetchUserInfo = async ({ username }: { username: string }) => {
    try {
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full md:w-3/4 mx-auto min-h-screen gap-4">
        ...
        {error.length > 0 ? (
          <h1 className="text-center text-[18px] font-semibold text-red-500 mt-44">
            {error}
          </h1>
        ) : (
          <section className="flex lg:flex-row flex-col mt-[36px] gap-12 p-4 md:p-0">
            <div className="flex flex-col w-full">
              <Profile
                ...
                bio={
                  userInfo?.userData?.bio?.length > 0
                    ? userInfo?.userData?.bio
                    : " "
                }
                tags={
                  userInfo?.userData?.tags?.length > 0
                    ? userInfo?.userData?.tags
                    : undefined
                }
        ...
        {!token && (
          <button
            className="flex items-center justify-center text-white rounded-full px-12 py-4 
